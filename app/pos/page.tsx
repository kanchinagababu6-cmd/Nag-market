...
export default function PosPage() {
  ...
  const handleBarcodeSubmit = (e: React.FormEvent) => {
    ...
    if (matched) {
      ...
      alert("Barcode not recognized in catalog!");
    }
  };
  ...
  const handleCheckout = async () => {
    ...
    try {
      const res = await fetch("/api/orders", {
        ...
        body: JSON.stringify({
          customerName: snapshotPhone ? `Counter Walk-in (${snapshotPhone})` : "Counter Walk-in",
          customerPhone: snapshotPhone || "0000000000",
  ...
  const getWhatsAppBillUrl = () => {
    if (!lastSale || !lastSale.phone) return "#";
    const cleanPhone = lastSale.phone.replace(/[^0-9]/g, "");
    const formattedPhone = cleanPhone.length === 10 ? `91${cleanPhone}` : cleanPhone;

    let text = `🧾 *NAG SUPERMARKET RECEIPT*\n`;
    text += `Bill #: *${lastSale.id.slice(0, 8).toUpperCase()}*\n`;
    text += `Time: ${lastSale.date}\n`;
    text += `--------------------------------\n`;
    lastSale.items.forEach((item) => {
      text += `• ${item.name}\n   ${item.quantity} x ₹${item.price.toFixed(2)} = *₹${(item.price * item.quantity).toFixed(2)}*\n`;
    });
    text += `--------------------------------\n`;
    text += `*Grand Total: ₹${lastSale.total.toFixed(2)}*\n`;
    text += `Mode: ${paymentType}\n\n`;
    text += `Thank you for shopping with Nag Supermarket! 🙏`;
    ...
    <div className="w-full max-w-7xl mx-auto p-4 space-y-5">
      ...
      {lastSale && (
        <div id="thermal-receipt" className="hidden print:block font-mono text-black text-xs max-w-[280px] p-2 leading-tight">
          ...
          <div>Bill: #{lastSale.id.slice(0, 8).toUpperCase()}</div>
          <div>Time: {lastSale.date}</div>
          <div className="border-t border-dashed border-black my-1"></div>
          <table className="w-full text-left text-[11px]">
            ...
            <tbody>
              {lastSale.items.map((i) => (
                <tr key={i.id}>
                  <td className="truncate max-w-[120px]">{i.name}</td>
                  <td>{i.quantity}</td>
          ...
          <div className="text-[10px] mt-1">Payment: {paymentType}</div>
          <div className="text-center text-[10px] mt-3">*** Thank You! Visit Again ***</div>
        </div>
      )}
      ...
      <div className="print:hidden space-y-5">
        ...
        <div className="flex justify-between items-center bg-slate-900 border border-slate-800 p-4 rounded-2xl">
          <div>
            ...
            <p className="text-xs text-slate-400">Barcode scanner, instant stock deduction & digital billing</p>
          </div>
          <div className="flex gap-2">
            <Link
              href="/admin/products"
              className="text-xs px-3 py-1.5 bg-slate-800 text-blue-400 hover:text-white rounded-xl border border-slate-700 font-semibold"
            >
              + Restock / Barcode
            </Link>
            <Link
        ...
        {/* Successful Sale Action Banner */}
        {lastSale && (
          <div className="p-4 rounded-2xl bg-emerald-500/10 border border-emerald-500/20 text-white text-xs flex flex-wrap justify-between items-center gap-3">
            <div>
              <span className="font-bold text-emerald-400">Bill Saved! </span>
              <span>ID: #{lastSale.id.slice(0, 8).toUpperCase()} | Total: <strong>₹{lastSale.total.toFixed(2)}</strong></span>
            </div>
            <div className="flex items-center gap-2">
              <button
                onClick={() => window.print()}
                className="px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-white rounded-xl border border-slate-700 font-semibold"
              >
                🖨️ Thermal Print Slip
              </button>
              {lastSale.phone && (
                <a
                  ...
                  className="px-3 py-1.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl font-semibold flex items-center gap-1 shadow"
                >
                  💬 Send WhatsApp Bill
                </a>
              )}
